...
const providerLabels = {
  ...
  paypal: "PayPal", global66: "Global66",
  transfer: "Transferencia", cash: "Manual",
}
...
const CustomTooltip = ({ active, payload, label, currency }) => {
  ...
  return (
    <div style={{
      ...
      <p style={{ color: "var(--text-2)", marginBottom: "6px" }}>{label}</p>
      {payload.map((p, i) => (
...
function ChartCard({ title, subtitle, children, span }) {
  return (
    <div style={{
      ...
      <p style={{ fontSize: "13px", fontWeight: 500, marginBottom: "4px" }}>{title}</p>
      {subtitle && (
...
export default function AdminAnalytics() {
  ...
  // Enriquece datos para charts
  const ordersChartData = orders_by_status.map(o => ({
    ...o,
    label: statusLabels[o.status] || o.status,
  }))

  const providerChartData = revenue_by_provider.map(p => ({
    ...p,
    ...
    <div style={{ padding: "clamp(24px, 4vw, 40px)", maxWidth: "1200px" }}>
      ...
      <div style={{ marginBottom: "32px" }}>
        ...
        <p style={{ fontSize: "13px", color: "var(--text-3)" }}>
          Últimos 12 meses — actualizado automáticamente cada 2 minutos.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3"
        ...
        <KpiCard
          ...
          value={`$${Number(kpis.total_revenue).toLocaleString("es-CL")}`}
          accent
        />
        <KpiCard
        ...
        <KpiCard
          ...
          sub="Clientes que repiten"
          accent={kpis.repeat_rate > 20}
        />
      </div>

      {/* Fila 1 — Ventas + Órdenes por estado */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5"
        style={{ marginBottom: "20px" }}>

        {/* Ventas por mes — barra */}
        <ChartCard
        ...
        >
          {sales_by_month.length === 0 ? (
            <p style={{ color: "var(--text-3)", fontSize: "13px", textAlign: "center",
              padding: "40px 0" }}>Sin datos todavía.</p>
          ) : (
        ...
        <ChartCard title="Órdenes por estado" subtitle="Distribución total">
          {ordersChartData.length === 0 ? (
            <p style={{ color: "var(--text-3)", fontSize: "13px",
              textAlign: "center", padding: "40px 0" }}>Sin datos.</p>
          ) : (
            <>
      ...
      {/* Fila 2 — Usuarios + Pagos por proveedor */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5"
        style={{ marginBottom: "20px" }}>

        {/* Nuevos usuarios por mes — línea */}
        <ChartCard title="Nuevos usuarios por mes" subtitle="Últimos 12 meses">
          {users_by_month.length === 0 ? (
            <p style={{ color: "var(--text-3)", fontSize: "13px",
              textAlign: "center", padding: "40px 0" }}>Sin datos todavía.</p>
          ) : (
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={users_by_month}
                margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
                <XAxis dataKey="month" tick={{ fontSize: 11, fill: "var(--text-3)" }}
                  axisLine={false} tickLine={false} />
        ...
        {/* Ingresos por método de pago — barras horizontales */}
        <ChartCard title="Ingresos por método de pago" subtitle="Pagos aprobados">
          {providerChartData.length === 0 ? (
            <p style={{ color: "var(--text-3)", fontSize: "13px",
              textAlign: "center", padding: "40px 0" }}>Sin pagos aprobados todavía.</p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              {providerChartData.map((p, i) => {
                const max = providerChartData[0]?.total || 1
      ...
      {/* Fila 3 — Categorías + Inscripciones */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5"
        style={{ marginBottom: "20px" }}>
        ...
        <ChartCard title="Ingresos por categoría" subtitle="Basado en ítems vendidos">
          {revenue_by_category.length === 0 ? (
            <p style={{ color: "var(--text-3)", fontSize: "13px",
              textAlign: "center", padding: "40px 0" }}>Sin ventas todavía.</p>
          ) : (
            <ResponsiveContainer width="100%" height={220}>
        ...
        {/* Inscripciones academia por mes */}
        <ChartCard title="Inscripciones academia" subtitle="Últimos 12 meses">
          {enrollments_by_month.length === 0 ? (
            <p style={{ color: "var(--text-3)", fontSize: "13px",
              textAlign: "center", padding: "40px 0" }}>Sin inscripciones todavía.</p>
          ) : (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={enrollments_by_month}
                margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
                <XAxis dataKey="month" tick={{ fontSize: 11, fill: "var(--text-3)" }}
                  axisLine={false} tickLine={false} />
      ...
      {/* Fila 4 — Productos con bajo stock */}
          ...
          <div style={{ display: "grid",
            ...
            {low_stock_products.map((p, i) => (
              <div key={i} style={{
                ...
                <span style={{
                  fontSize: "14px", fontWeight: 600, marginLeft: "12px", flexShrink: 0,
